...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (lastVisitedPlanet) {
    const targetBtn = document.querySelector(
      `.planet-btn[data-target="${lastVisitedPlanet}"]`
    );
    if (targetBtn) {
  ...
  function updateMoonCarousel() {
    ...
    moonCarousel.style.transform = `translateX(-${
  ...
  messageBtn.addEventListener("click", () => {
    ...
    if (message) {
      ...
      messageElement.className =
        "message-bubble animate__animated animate__fadeInUp";
  ...
  function createSpecialShootingStar() {
    ...
    specialStar.className =
      "shooting-star special animate__animated animate__faster";
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    document.querySelectorAll(".planet").forEach((planet) => {
      const speed = parseFloat(planet.dataset.speed) || 0.8;
      const x = mouseX * speed;
      const y = mouseY * speed;
      planet.style.transform = `translate(${x}px, ${y}px) 
            rotate(${x * 0.3}deg) 
            scale(${1 + Math.abs(x * 0.005)})`;
    });
  });
  ...
  function animateWave() {
    ...
    waves.forEach((wave, index) => {
      const height = 10 + Math.abs(Math.sin(Date.now() / 200 + index) * 20);
      wave.style.height = `${height}px`;
      wave.style.opacity =
        0.3 + Math.abs(Math.sin(Date.now() / 200 + index)) * 0.7;
    });
